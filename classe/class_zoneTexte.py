...
class ZoneDeTexte():
    def __init__(self,ecran,x,y,largeur,hauteur,couleurTexte,couleurFond,couleurBordure,tailleBordure,borderRadius=0,nbLettreMax=None,actif=True,desactivate=True,cara_excle=[],censure=False):
        """methode pour initialiser les parametre de la class ZoneDeTexte"""
        self.ecran=ecran
        self.x=x
        self.y=y
    ...
    def _updateTexteSurface(self):
        """methode pour mettre a jour les texte"""
        if self.censure:
    ...
    def dessiner(self):
        """methode pour dessiner le texte"""
    ...
    def changeText(self,texte):
        """methode pour modifier le texte de la zone avec un nouveau texte"""
        self.texte=texte
        self._updateTexteSurface()

    def addText(self,event):
        """methode pour ajouter une lettre au texte"""
        if not self.actif:
            return
        texteAvant=self.texte
        if event.key==pygame.K_BACKSPACE:#supprime un caractere
            if len(self.texte)>0:
                self.texte=self.texte[:-1]
        elif event.unicode.isprintable() and event.unicode in self.cara_excle: #verifier si le caractere ne fait pas partie de la liste des caractere exclu
        ...
        elif event.unicode.isprintable() and len(self.texte)<self.nbLettreMax:#joute la lettre au texte tant que le texte ne depasse pas le nombre de texte max
            self.texte+=event.unicode
        elif event.unicode.isprintable():#affiche un warning si le nombre de caractere depasse la limite autoriser
    ...
    def checkClick(self,pos):
        """methode pour verifier si la zone est cliquer"""
    ...
    def _desactiver(self):
        """methode pour desactiver la zone"""
...
class AfficheMots():
    def __init__(self,ecran,x,y,largeur,hauteur,nbLettre,couleurTexte):
        """methode pour initialiser les parametre de la class AfficheMots"""
    ...
    def initialiseMot(self):
        """methode pour initialiser la mots (que des _ au debut)"""
        self.mot="_ "+"_ "*(self.nbLettre-2)+"_"
        self.surface_texte=self.font.render(self.mot,True,self.couleurTexte)
    def update_text_surface(self):
        """methode pour mettre a jour le mot"""
        self.surface_texte=self.font.render(self.mot,True,self.couleurTexte)
    
    def dessiner(self):
        """methode pour dessiner le mots"""
    ...
    def changeMot(self,mot):
        """methode pour modifier le mots à afficher"""
        self.mot=mot
        self.update_text_surface()
